...
namespace ArfFipaso.Filter.Extensions
{
	public static class FilterExtensions
	{
		...
		public static IQueryable<T> Filter<T>(this IQueryable<T> query, List<XFilterItem> filterItems) where T : class
		{
			...
			{
				var (baseParameterExpression, baseMemberExpression, listParameterExpression, listMemberExpression) = ExpressionUtils.GetExpressions(typeof(T), filterItem.Key);
				...
				{
					var outherLambda = ExpressionUtils.GetListLambda(baseParameterExpression, baseMemberExpression, listParameterExpression, lambda);
